...
module "redis" {
  ...
  replication_group_id        = "redis-cluster-prod-ms"
  description                 = "This redis cluster will be used by prod MS ."
  engine_version              = "7.1"
  node_type                   = "cache.t3.micro"
  num_cache_clusters          = 3 # Replace with the actual number of nodes
  parameter_group_name        = "default.redis7.cluster.on"
  port                        = 6379
  subnet_group_name           = aws_elasticache_subnet_group.redis_subnet_group-prod.name # You can provide your already created subnet group name also
...
resource "aws_cloudwatch_log_group" "redis_slow_log" {
  name              = "redis-cluster-prod-ms-slow-logs"
  retention_in_days = 14 # Adjust retention as needed
}

resource "aws_cloudwatch_log_group" "redis_engine_log" {
  name              = "redis-cluster-prod-ms-engine-logs"
  retention_in_days = 14 # Adjust retention as needed
}

# Create Security Group
resource "aws_security_group" "redis_sg" {
  name        = "redis-cluster-prod-ms-sg"
...
# Create Subnet Group
resource "aws_elasticache_subnet_group" "redis_subnet_group-prod" {
