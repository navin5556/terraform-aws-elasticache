locals {
  default_mumbai_vpc = "vpc-02ddc0f45bd9cf6bc" # Replace with your VPC ID of PROD env
  default_mumbai_subnet = ["subnet-0e0fc981bf2b53729","subnet-04e0cee3bb32c362b","subnet-0e55a335d8f4d810b"] # Replace with your subnet IDs of PROD env
...
module "redis" {
  ...
  replication_group_id        = "redis-cluster-uat-ms"
  description                 = "This redis cluster will be used by uat MS ."
  engine_version              = "7.1"
  node_type                   = "cache.t3.micro"
  num_cache_clusters          = 3 # Replace with the actual number of nodes
  parameter_group_name        = "default.redis7.cluster.on"
  port                        = 6379
  subnet_group_name           = aws_elasticache_subnet_group.redis_subnet_group-uat.name # You can provide your already created subnet group name also
...
resource "aws_cloudwatch_log_group" "redis_slow_log" {
  name              = "redis-cluster-uat-ms-slow-logs"
  retention_in_days = 14 # Adjust retention as needed
}

resource "aws_cloudwatch_log_group" "redis_engine_log" {
  name              = "redis-cluster-uat-ms-engine-logs"
  retention_in_days = 14 # Adjust retention as needed
}

# Create Security Group
resource "aws_security_group" "redis_sg" {
  name        = "redis-cluster-uat-ms-sg"
...
# Create Subnet Group
resource "aws_elasticache_subnet_group" "redis_subnet_group-uat" {
  name       = "redis-subnet-group-uat"
